...
Current release candidate: **0.9.35**. See the [bilingual release notes](docs/releases/0.9.35.md).

> **上线状态 / Launch status:** 正式域名、认证邮件、账号隔离同步、数据库迁移、生产监控和每日 R2 加密异地备份已经启用。当前公开候选版本为 `0.9.35`；Chrome 网上应用店仍在审核 `0.9.32`，审核状态以开发者后台为准，审核完成后再提交当时的最新版本。在商店恢复上架前，可使用在线版或 GitHub Release。重要数据仍建议保留 JSON 数据备份；动态壁纸视频只保存在当前设备，不嵌入备份。
>
> The production domain, authentication email, account-isolated synchronization, database migrations, monitoring, and daily encrypted R2 backups are active. The current public release candidate is `0.9.35`; the Chrome Web Store is still reviewing `0.9.32`, and the latest release available at that time will be submitted after the review completes. The developer dashboard remains the source of truth for review status. Until the listing is restored, use the hosted app or the GitHub Release. Keep a JSON data backup for important data; dynamic wallpaper video bytes stay on the current device and are not embedded in the backup.
...
- Import and backup: supports WhyNavo JSON, WeTab `.data`, browser bookmarks HTML, CSV, Markdown note export, and old new-tab page migration input.
...
- Static image and device-local MP4/WebM dynamic wallpaper upload
...
│   │   ├── ui-v040.css          # Global and responsive styles
...
The hosted app receives new versions through Cloudflare Pages. Refresh the page when an important update notice appears. A service worker may keep the current verified version available while offline, but the app does not silently continue cloud synchronization when the server has retired an unsafe client protocol.
...
The production email path uses the verified `auth.whynavo.com` sender domain with Resend and the signed Supabase Send Email Hook. DKIM, SPF, return-path MX, and DMARC are published in Cloudflare DNS; the provider key is restricted to that sender domain. The production Hook and the tracked bilingual templates route one-time tokens through `confirm.html` so mailbox link prefetch cannot consume them before the user explicitly continues.
