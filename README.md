...
- Chrome extension searches start with Chrome's Search API and the default provider selected by the user. The search field also offers an explicitly chosen Baidu option for an individual query; WhyNavo never changes or persists Chrome's default provider.
...
- Chrome/Edge 新标签页搜索默认使用浏览器默认搜索引擎；用户可以在搜索框内主动选择百度进行单次搜索，WhyNavo 不修改或持久化 Chrome 的默认搜索设置。
