...
import { accountScopedKey, commitAnonymousStateAdoption, deleteKey, deleteLocalAccountData, downloadJson, loadStateForAccount, mergeAndSaveStateForAccount, readKey, saveStateForAccount, writeKey } from "./db";
...
const pendingAccountCleanupIds = () => {
  try {
    const value = JSON.parse(localStorage.getItem(PENDING_ACCOUNT_CLEANUP_KEY) || "[]");
...
const setAccountCleanupPending = (userId: string) => {
  try {
    localStorage.setItem(
      PENDING_ACCOUNT_CLEANUP_KEY,
      JSON.stringify(Array.from(new Set([...pendingAccountCleanupIds(), userId])))
    );
    return true;
  } catch {
    return false;
...
const clearAccountCleanupPending = (userId: string) => {
  try {
    const remaining = pendingAccountCleanupIds().filter((candidate) => candidate !== userId);
    if (remaining.length) localStorage.setItem(PENDING_ACCOUNT_CLEANUP_KEY, JSON.stringify(remaining));
    else localStorage.removeItem(PENDING_ACCOUNT_CLEANUP_KEY);
  } catch {
...
const cleanupDeletedAccountData = async (userId: string) => {
  ...
  clearAccountCleanupPending(userId);
};
...
export default function App() {
  ...
  useEffect(() => {
    void (async () => {
      for (const userId of pendingAccountCleanupIds()) {
        await cleanupDeletedAccountData(userId).catch(() => undefined);
      }
  ...
  useEffect(() => {
    ...
    channel.onmessage = (event: MessageEvent<{
      ...
      if (message.type === "account-deleted" && messageUserId) {
        setAccountCleanupPending(messageUserId);
      ...
      {dialog === "sync" && (
        <SyncDialog
          ...
          onDeleteAccount={async (password, captchaToken) => {
            ...
            const cleanupQueued = setAccountCleanupPending(deletingUserId);
