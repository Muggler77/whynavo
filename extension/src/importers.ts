...
export function parseBookmarksHtml(html: string): ImportShortcut[] {
  ...
  return Array.from(doc.querySelectorAll("a[href]"))
    .slice(0, MAX_IMPORTED_SHORTCUTS)
    .flatMap((anchor) => {
      const url = cleanUrl((anchor as HTMLAnchorElement).getAttribute("href") || "");
      if (!url) return [];
      return [{
        title: cleanLabel(anchor.textContent) || new URL(url).hostname.slice(0, MAX_IMPORTED_LABEL_CHARS),
        url,
        iconUrl: cleanIconUrl((anchor as HTMLElement).getAttribute("icon"))
      }];
    })
