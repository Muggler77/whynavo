...
try {
  ...
  const { MAX_IMPORTED_SHORTCUTS, faviconHostFor, importedToShortcuts, normalizeIconReference, parseImportText } = await import(pathToFileURL(importersOutput).href);
  ...
  assert.doesNotMatch(weatherSource, /google\.com\/search/, "weather source links must not disclose location coordinates to an additional search provider");
  ...
  assert.match(appSource, /PENDING_ACCOUNT_CLEANUP_KEY[\s\S]*cleanupDeletedAccountData/, "failed local account deletion cleanup must remain queued for the next startup");
  assert.match(appSource, /for \(const userId of pendingAccountCleanupIds\(\)\)[\s\S]*cleanupDeletedAccountData\(userId\)/, "startup must retry account data cleanup after a prior browser storage failure");
  assert.match(
    appSource,
  ...
  assert.match(
    ...
    "manual production redeploys must require the matching GitHub Release"
  );
  ...
  assert.match(
    releaseWorkflow,
    /deploy:supabase-migrations -- --through 0012[\s\S]*Deploy backward-compatible Edge Functions[\s\S]*Publish GitHub Release/,
    "the release artifact must publish only after its backward-compatible database and function dependencies are ready"
  );
  assert.match(
    releaseWorkflow,
  ...
  assert.match(
    ...
    "the hosted version must switch only after the matching release artifact is public"
  );
