...
- The release workflow completes all checks and builds first, prepares backward-compatible database changes and Edge Functions, and publishes the matching verified installation artifact. Only then does the called production workflow switch the Pages bundle and revoke the unsupported sync API. A normal `main` push cannot expose a version manifest before its matching release exists.
