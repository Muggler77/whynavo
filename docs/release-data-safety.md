...
7. If the sync protocol changed, prepare a backward-compatible server API first, publish and verify the client, then apply a separate migration that revokes the retired API. Run two-device concurrent-write tests.
...
- Publish the verified extension artifact only after the backward-compatible migrations and Edge Functions are ready. Switch the hosted `latest-version.json` only after that artifact exists; ordinary `main` pushes must not create a window in which the update button points to an older or missing release.
