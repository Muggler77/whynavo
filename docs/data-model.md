...
主页“我的网站”与 Dock 使用不同字段：`homeVisible` 决定网站或文件夹是否进入最多 12 项的主页清单，`homeX/homeY` 保存归一化自由位置，`pinned` 只控制 Dock。`settings.homeSelectionInitialized` 用于区分旧用户的前 12 项兼容视图和用户已经主动维护过的主页清单。
