...
5. 创建 Cloudflare Turnstile widget，允许官方 `whynavo.pages.dev` 主机名，把站点密钥配置为 `VITE_TURNSTILE_SITE_KEY`，并在 Supabase Auth 的 Bot and Abuse Protection 中启用 Turnstile。
...
4. 脚本会把可见链接复制成 JSON。
5. 回到 WhyNavo，点击“导入”，粘贴 JSON 并确认。

如果浏览器阻止读取或旧页面不是普通链接结构，可以导入浏览器书签 HTML、CSV 或 WhyNavo JSON。

## 备份

WhyNavo 默认把数据存在本机 IndexedDB。建议在初次整理好快捷导航后点击“导出”，保留 JSON 备份。开启 Supabase 后，数据会同时存在本机和云端。
